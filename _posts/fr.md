...
## Détectez les erreurs de vos fichiers de paiement avant votre banque

**Pain001** est une suite Python open source pour l'initiation de paiements ISO 20022. Elle convertit les données que vous avez déjà (CSV, Excel, SQLite, JSON, Parquet, ou messages SWIFT MT101 hérités) en XML [`pain.001`](/fr/pain-001/) (virements) et `pain.008` (prélèvements) validés par les schémas XSD officiels.
...
Validez un lot d'exemple directement dans votre navigateur, sans rien téléverser, puis prouvez le résultat avec le schéma XSD officiel : [essayer Pain001](/try/). Des fichiers CSV d'exemple sont téléchargeables comme modèles pour vos propres exports.

## L'échéance des adresses structurées, reportée

Depuis la fin de la coexistence MT–MX le 22 novembre 2025, l'échéance suivante était fixée au 14 novembre 2026 : CBPR+ devait rejeter les adresses postales entièrement non structurées, et le relais interbancaire MT101 devait être remplacé par `pain.001` version 9. En août 2026, Swift a reporté ces deux changements et annoncera de nouvelles dates d'ici décembre 2026 ; les exigences elles-mêmes demeurent. Pain001 gère déjà les adresses structurées et hybrides, et convertit vos MT101 en une commande.
