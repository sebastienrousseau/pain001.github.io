...
**Schema provenance.** The copy of `pain.001.001.03` shipped with Pain001 carries **no generation header**, so its origin cannot be evidenced from the file itself — it validates correctly, but treat the [ISO 20022 archive](https://www.iso20022.org/catalogue-messages/iso-20022-messages-archive?search=pain.001.001.03) as authoritative and check against it before relying on this reference for compliance work.
