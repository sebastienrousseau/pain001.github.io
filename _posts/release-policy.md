...
The honest pre-1.0 statement: **the latest release is the supported release.** Bug and security fixes ship as new releases rather than backports. If you need a frozen version, pin it (`pain001==0.0.71`). Pinned installs keep working because releases are never deleted or mutated on PyPI.
