...
GEN_RE = re.compile(
    r"Generated by ([^(]+?)(?:\s*\(build:([^)]+)\))? on "
    r"(\d{4} \w{3} \d{1,2})", re.I)


def schema_provenance(xsd: Path) -> dict:
    """Read the generation header ISO's Standards Editor writes.

    This is evidence from the file itself, not a claim about the ISO
    publication date — the two differ (pain.001.001.13 was generated
    2026-03-02 and published later that month). Four of the shipped
    schemas carry no such header at all, and saying so is more useful
    than inventing a date for them.
    """
    head = xsd.read_text(encoding="utf-8", errors="replace")[:600]
    m = GEN_RE.search(head)
    if not m:
        return {"generator": None, "build": None, "generated": None,
                "header": None}
    return {
        "generator": m.group(1).strip(),
        "build": (m.group(2) or "").strip() or None,
        "generated": m.group(3),
...
def main() -> int:
    ...
    for d in sorted(templates.glob("pain.*")):
        ...
        payload = {
            "version": version,
            "provenance": schema_provenance(xsd),
