...
def render_version(payload: dict, index: dict) -> None:
    ...
    if populated:
        ...
        body.append(
            ...
            f"{len(populated)} types in all. The tree above repeats a party or "
...
def render_types(payload: dict) -> None:
    ...
    fm = load_frontmatter(
        ...
        f"official XSD.",
        "Message specification",
        f"Each of the {len(populated)} types in {v}, defined once. This is "
        "the view to use for field mapping.",
...
def render_types_chunked(payload: dict) -> list[str]:
    ...
    for i, (chunk, slug) in enumerate(zip(chunks, slugs, strict=False)):
        ...
        fm = load_frontmatter(
            ...
            "Message specification",
            f"Types {span} in {v}, defined once each. This is the view to use "
            "for field mapping.",
            f"{v} types, ISO 20022 complex types, cardinality, field mapping",
        )
        write_post(slug, fm, "\n".join(body))
...
def render_hub(specs: dict, index: dict) -> None:
    ...
    fm = load_frontmatter(
        ...
        "supported pain.001 and pain.008 version, generated rather than "
        "transcribed.",
